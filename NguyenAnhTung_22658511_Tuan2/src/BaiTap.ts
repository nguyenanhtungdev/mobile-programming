...
async function tripleAfter2s(num: number): Promise<number> {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve(num * 3);
    }, 1000);
...
async function runSequential() {
  ...
  const first = await tripleAfter2s(2);
  console.log("First result:", first);

  const second = await tripleAfter2s(first);
  console.log("Second result:", second);

  const third = await tripleAfter2s(second);
...
function simulateTask5(value: string, time: number): Promise<string> {
  return new Promise((resolve) => {
...
async function run5() {
  const tasks = [
    simulateTask5("A", 1000),
    simulateTask5("B", 500),
    simulateTask5("C", 1500),
  ];
...
function simulateTask4(id: number, time: number): Promise<string> {
  return new Promise((resolve) => {
    setTimeout(() => {
...
async function batchProcess() {
  ...
  const tasks = [
    ...
    simulateTask4(4, 3000),
    simulateTask4(5, 2500),
  ];
...
function simulateTask3(id: number, time: number): Promise<string> {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve(`Task ${id} done in ${time}ms`);
    }, time);
...
async function batchFetchTodos() {
  ...
  const results = await Promise.allSettled(ids.map((id) => fetchTodo2(id)));
